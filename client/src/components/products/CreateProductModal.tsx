...
import { ProductItemType } from "./ProductsTable";
...
const CreateProductModal = ({
  ...
  const setType = (e: ChangeEvent<HTMLSelectElement>) => {
    ...
    if (MAPPER_PRODUCT_TYPE.hasOwnProperty(selectOption)) {
      setCreateModalData((oldData) => ({
        ...oldData,
        type: MAPPER_PRODUCT_TYPE[
          selectOption as keyof typeof MAPPER_PRODUCT_TYPE
        ],
      }));
    }
  };
  const setSizePerUnit = (value: string) =>
    setCreateModalData((oldData) => ({
      ...oldData,
      sizePerUnit: Number(value),
    }));

  const onConfirmCreate = async () => {
    const productToCreate = {
      name: createModalData.name || "",
      sizePerUnit: createModalData.sizePerUnit || 0,
      type: createModalData.type || "hazardous",
    };

    const result = await createProduct({ variables: productToCreate });
    ...
    >
      <div className="relative top-40 mx-auto shadow-xl rounded-md bg-white max-w-md">
        ...
        <div className="flex flex-col gap-y-5 dir p-6 pt-0">
          <TextInput text="Product name" onChange={setName} />
          <div className="peer w-full h-full bg-transparent text-blue-gray-700 font-sans font-normal outline outline-0 focus:outline-0 disabled:bg-blue-gray-50 disabled:border-0 transition-all placeholder-shown:border placeholder-shown:border-blue-gray-200 placeholder-shown:border-t-blue-gray-200 border focus:border-2 border-t-transparent focus:border-t-transparent text-sm px-3 py-2.5 rounded-[7px] border-blue-gray-200 focus:border-blue-500">
            <select onChange={setType} className="w-full">
